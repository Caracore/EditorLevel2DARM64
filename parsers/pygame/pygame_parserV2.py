...
class EditorLevelLoader:
    ...
    def _load_tilesets(self):
        ...
        if isinstance(self.tilesets, list):
            # Si c'est une liste, utiliser le champ "id" de chaque tileset
            for tileset_info in self.tilesets:
                tileset_id = tileset_info.get("id", 0)
        ...
        elif isinstance(self.tilesets, dict):
            # Si c'est un dictionnaire
            for tileset_id, tileset_info in self.tilesets.items():
                tileset_path = tileset_info["path"]
                full_path = project_dir / tileset_path
    ...
    def get_tile_surface(self, tile_data: Dict) -> Optional[pygame.Surface]:
        ...
        if "Color" in tile_data:
            ...
            tileset_id = texture_info["tileset_id"]  # Garder en int
            ...
            if tileset_id in self.tileset_surfaces:
                ...
                tileset_info = None
                if isinstance(self.tilesets, list):
                    # Chercher le tileset par son id
                    for ts in self.tilesets:
                        if ts.get("id") == tileset_id:
                            tileset_info = ts
                            break
                elif isinstance(self.tilesets, dict):
                ...
                if tileset_info:
                    ...
                    # Support de "tiles_per_row" ou "columns"
                    tiles_per_row = tileset_info.get("tiles_per_row", tileset_info.get("columns", 1))
                    tile_width = tileset_info.get("tile_width", 16)
                    tile_height = tileset_info.get("tile_height", 16)
